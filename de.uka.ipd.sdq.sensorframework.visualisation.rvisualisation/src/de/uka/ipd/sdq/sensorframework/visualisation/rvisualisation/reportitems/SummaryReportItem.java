...
/**
 * Summarizes the values stored in a R variable by displaying some statistical measures. These
 * measures are Minimum, 1st Quartile, Median, Mean, 3rd Quartile, Maximum.
 * 
 * @author groenda
 */
public class SummaryReportItem extends RCommandRReportItem {
     ...
     * 
     * @param rVariableName
     *            Name of the R variable in which the data is stored.
     * @param displayName
     *            Name displayed when referring to the variable.
     */
    public SummaryReportItem(final String rVariableName, final String displayName) {
        super("summary(" + rVariableName + ")", "Summary of Sensor " + displayName);
    }
    ...
    public String getText() {
        ...
        if (returnedValue.rtype == REXP.REALSXP) {
            ...
            for (int measureNumber = 0; measureNumber < 6; measureNumber++) {
                ...
                df.applyPattern("###.0");
                if (Math.abs(doubleArray[measureNumber]) < 0.2)
                    df.applyPattern("#0.0#E0#");
                if (Math.abs(doubleArray[measureNumber]) > 999)
                    df.applyPattern("#0.0#E0#");
