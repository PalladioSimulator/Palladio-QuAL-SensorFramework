...
public abstract class AbstractReportView extends EditorPart implements ITabbedPropertySheetPageContributor, Observer {
    ...
    private void addDropSupport(Control control) {
        ...
        target.addDropListener(new ViewDropTargetListener(this.getEditorInput()));
    }
        ...
        for (ConfigEntry re : myInput.getConfigEntrys()) {
            for (Sensor s : re.getSensors()) {
                try {
                    ...
                    showMessage(s.getSensorName(), "Missing the Measurements of sensor!");
                }
            }
        }
        // get the adapter factory for input
        IAdapterFactory adapterFactory = AdapterRegistry.singleton().getFactoryByID(myInput.getAdapterFactoryID());

        /*
         * Add the data sources to this report an observe changes at the adapters and their
         * settings.
         */
    ...
    private void showMessage(String title, String message) {
        MessageDialog.openInformation(this.getEditorSite().getShell(), title, message);
    }
