...
/**
 * This class offers the methods, which it for validating possible a Sensor->View makes.
...
public class SensorValidationToView {
        ...
        for (IConfigurationElement element : elements) { // Iterate over all registered views
            ...
            try {
                ...
                if (AdapterRegistry.singleton().canAdapt(sensorAndMeasurements, viewerAcceptsClass)) {
                    ...
                    for (IAdapterFactory f : AdapterRegistry.singleton().getAllAvailableFactories(
                            sensorAndMeasurements, viewerAcceptsClass)) {
    ...
    public static boolean canViewSensor(SensorAndMeasurements sensorAndMeasurements) {
        ...
        String activeEditorId = VisualisationPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow()
    ...
    public static void showMessage(Shell shell) {
        ...
        String editorName = VisualisationPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage()
...
class DialogLabelProvider extends LabelProvider {
    ...
    public String getText(Object element) {
        if (element instanceof ViewAndAdapterFactory) {
            ...
            return displayName.replace("{0}", viewAndAdapter.getFactory() == null ? "" : viewAndAdapter.getFactory()
                    .getMetricLabel());
        }
