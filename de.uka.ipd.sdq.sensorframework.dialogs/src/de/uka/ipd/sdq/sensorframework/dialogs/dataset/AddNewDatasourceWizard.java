...
public class AddNewDatasourceWizard extends Wizard {
    ...
    public void addPages() {
        ...
        selectTypePage = new WizardSelectDatasourcePage("Select Type of Datasource to create", true, true, true);
    ...
    public boolean performFinish() {
        ...
        if (selectTypePage.getResult().equals("Memory Datasource")) {
            ...
            IPath targetDirectory = ResourcesPlugin.getWorkspace().getRoot().getFile(selectedWorkspacePath)
                    .getLocation();
            try {
                IDAOFactory fileFactory = new FileDAOFactory(targetDirectory.toOSString());
                SensorFrameworkDataset.singleton().addDataSource(fileFactory);
            } catch (Exception ex) {
                MessageDialog.openError(getShell(), "File DAO factory error.", ex.getMessage());
