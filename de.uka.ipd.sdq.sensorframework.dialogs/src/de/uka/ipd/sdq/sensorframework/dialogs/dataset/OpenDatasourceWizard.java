...
import de.uka.ipd.sdq.sensorframework.model.dao.EMFDAOFactory;
...
public class OpenDatasourceWizard extends Wizard {
	...
	private WizardDatasourceLoadPage db40DatasourceLoadPage, fileDatasourceLoadPage, emfDatasourceLoadPage;
	...
	public void addPages() {
		...
		selectDatasourceTypePage = new WizardSelectDatasourcePage(
				"Select Type of Datasource to load", false, true, true, true);
		...
		emfDatasourceLoadPage = new WizardDatasourceLoadPage(
				"Open the EMF folder", IResource.FOLDER);
		this.addPage(emfDatasourceLoadPage);
	}
	...
	public boolean performFinish() {
		...
		if (selectDatasourceTypePage.getResult().equals(
			...
			try {
				...
				WizardSelectDatasourcePage.EMF_DATASRC)) {
			path = emfDatasourceLoadPage.getFileFullPath();

			try {
				factory = new EMFDAOFactory(path.toOSString());
				SensorFrameworkDataset.singleton().addDataSource(factory);
			} catch (Throwable e) {
				MessageDialog.openError(getShell(), "EMF DAO factory error.",
						e.getMessage());
				SensorFrameworkDialogPlugin.log(IStatus.ERROR, e.getMessage());
				return false;
			}
	...
	public boolean canFinish() {
		if (selectDatasourceTypePage.getResult().equals(
			...
			return fileDatasourceLoadPage.isPageComplete();
				...
				WizardSelectDatasourcePage.EMF_DATASRC)) {
			return emfDatasourceLoadPage.isPageComplete();
		}
		return false;
	}
