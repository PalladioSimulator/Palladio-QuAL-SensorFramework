...
public abstract class StateSensorToPieAdapter extends DataAdapter {
     ...
     * 
     * @param timeSums
     *            A map which associates to each state the sum of the time this state has been
     *            active
     * @param sum
     *            Total sum of times
     * @return A pie representation of the data
     */
    private AbstractPie createPie(final HashMap<String, Double> timeSums, final double sum) {
        AbstractPie p = instantiatePie(((StateSensor) samInformation.getSensor()).getSensorName());
     ...
     */
    protected abstract double calculateFractions(final HashMap<String, Double> fractions);
