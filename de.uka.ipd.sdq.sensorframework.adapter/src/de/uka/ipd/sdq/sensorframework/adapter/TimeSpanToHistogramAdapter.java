...
public class TimeSpanToHistogramAdapter extends DataAdapter {
     ...
     *            Information about the TimeSpanSensor and the measurements.
     */
    public TimeSpanToHistogramAdapter(final SensorAndMeasurements sensorAndMeasurements) {
        super();
        this.samInformation = sensorAndMeasurements;

        /*
         * Check if there would be at least to different buckets for the histogram as JFreeChart
        ...
        if (maxValue - minValue < Histogram.DEFAULT_BUCKET_WIDTH) {
            ...
            this.adapterProperties.put(HISTOGRAM_WIDTH, (maxValue - minValue) / 2.0);
        } else {
            ...
            if (currentNumberOfBuckets > Histogram.MAXIMUM_NUMBER_OF_BUCKETS) {
                ...
                // Also, only use whole numbers for bucket width. Thus, round up after applying the
                // factor.
                ...
                this.adapterProperties.put(HISTOGRAM_WIDTH, newNumberOfBuckets);
            } else {
    ...
    public Object getAdaptedObject() {
        ...
        Histogram histogram = new Histogram(samInformation.getSensor().getSensorName(), histWidth);
        HashMap<Integer, Integer> histClasses = new HashMap<Integer, Integer>();

        int maxHistClass = assignMeasurementsToClasses(histClasses, histWidth);
        storeClassifiedMeasurementsInHistogram(histogram, histClasses, maxHistClass);
     ...
     * 
     * @param histogram
     *            The histogram in which the information is stored.
     * @param histClasses
     *            The classes and the frequency of the measurements per class.
     * @param maxHistClass
     *            The class with the higher number in which the frequency is greater than 0.
     */
    private void storeClassifiedMeasurementsInHistogram(Histogram histogram,
            final HashMap<Integer, Integer> histClasses, final int maxHistClass) {
        ...
        for (int i = 0; i <= maxHistClass; i++) {
            ...
            if (histClass == null) {
                /*
                 * only append classes with 0 probability if there is at least one class with
                 * probability >=0 in the left hand side of the current class.
                 */
                if (firstValueFound) {
                    histogram.addEntity(new HistogramBucketInformation(0.0, i * histWidth));
                }
            } else {
    ...
    /**
     * Assigns the measurement to histogram classes. The number of measurements is stored for each
     * of the classes.
     * 
     * @param histClasses
     *            Receives the number of measurements for each class.
     * @param histWidth
     *            width of the classes of a histogram.
     * @return number of the highest class.
     */
    private int assignMeasurementsToClasses(HashMap<Integer, Integer> histClasses, final double histWidth) {
        int maxHistClass = 0;
        int histogramClassNumber;
        for (Measurement measurement : samInformation.getMeasurements()) {
            TimeSpanMeasurement tsm = (TimeSpanMeasurement) measurement;
            /*
             * checks to which class the measurement belongs. The class number always starts at 0,
             * which means the interval [0..histWidth/2). Uses integer rounding and comparison for
            ...
            if (classNumber == null) {
                ...
                histClasses.put(histogramClassNumber, classNumber.intValue() + 1);
            }
            /*
             * Store the number of the highest class in which a measurement was found
             */
