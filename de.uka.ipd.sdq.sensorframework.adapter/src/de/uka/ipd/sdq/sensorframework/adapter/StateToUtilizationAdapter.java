...
/**Adapter converting {@link StateMeasurement}, e.g. for CPU Resource consumption, to Utilization measurements.
 * @author groenda
 */
public class StateToUtilizationAdapter extends DataAdapter {
	 ...
	 */
	public StateToUtilizationAdapter(
	...
	protected void initializeUtilizationInterval() {
		 ...
		 * Utilization as JFreeChart otherwise displays a bar with default
		 * width and the small values are very difficult to identify.
		 */
		if (samInformation.getMeasurements().size() <= 1) {
			this.adapterProperties.put(UTILIZATION_WIDTH, 
					Utilization.DEFAULT_BUCKET_WIDTH);
			return;
		}
		...
		for (Measurement measurement : samInformation.getMeasurements()) {
			...
			minEventTimeValue = (minEventTimeValue < stateMeasurement.getEventTime()) 
					? minEventTimeValue : stateMeasurement.getEventTime();  
			maxEventTimeValue = (maxEventTimeValue > stateMeasurement.getEventTime()) 
					? maxEventTimeValue : stateMeasurement.getEventTime();  
		}
		if (maxEventTimeValue - minEventTimeValue < Utilization.DEFAULT_BUCKET_WIDTH) {
			//whole utilization would be in one bucket, thus set bucket width to 1/2 of total width.  
			this.adapterProperties.put(UTILIZATION_WIDTH, 
					(maxEventTimeValue - minEventTimeValue) / Utilization.DEFAULT_NUMBER_BUCKETS);
		} else {
			...
			if (currentNumberOfBuckets > Utilization.MAXIMUM_NUMBER_OF_BUCKETS){
				this.adapterProperties.put(UTILIZATION_WIDTH, 
					(maxEventTimeValue - minEventTimeValue) / Utilization.MAXIMUM_NUMBER_OF_BUCKETS);
			} else {
				this.adapterProperties.put(UTILIZATION_WIDTH, 
						Utilization.DEFAULT_BUCKET_WIDTH);
			}
	...
	public Object getAdaptedObject() {
		...
		for (Measurement measurement : measurements) {
			...
			// process
			if (stateMmt.getEventTime() >= (currentInterval + 1) * utilWidth) { // measurement belongs to subsequent bucket. Finish current bucket 
				// finish current bucket
				if (!lastStateIsIdle) {
					currentUtilization += (currentInterval + 1) * utilWidth - lastEventTime;
				}
				utilization.addEntity(new UtilizationBucketInformation(currentUtilization / utilWidth, currentInterval * utilWidth));
				currentInterval++;
				// finish buckets until the one containing the current measurement is reached
				while (stateMmt.getEventTime() > (currentInterval + 1) * utilWidth) { // bucket without measurements
					if (lastStateIsIdle) {
