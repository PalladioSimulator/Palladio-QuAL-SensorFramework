...
public class RCommandRReportItem extends AbstractGeneratedTextRReportItem {
     ...
     * 
     * @param rCommands
     *            R commands that lead to the calculation of the statistical value.
     * @param description
     *            Description of this report item.
     */
    public RCommandRReportItem(final String rCommands, final String description) {
        super(description);
    ...
    public void generateData(final RConnection rConnection) {
        ...
        if (!result.isEmpty()) {
            ...
            RVisualisationPlugin.log(IStatus.WARNING, "The calculation of the R command report item \""
                    + getDescription() + "\" yield to no result. "
                    + "Most probably data import or the calculation in R " + "went wrong. Details: lastConsoleMessage="
