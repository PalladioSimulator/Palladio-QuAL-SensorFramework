...
public class PdfReportItem extends AbstractPlotReportItem {
    ...
    /**
     * Constructs a new report item containing a plotted pdf graphic. The graphic is stored in a
     ...
     *            Label for the x axis.
     */
    public PdfReportItem(final String title, final int height, final int width, final int fontSize,
    ...
    protected String generatePlotCommand() {
        ...
        for (String id : getDataSeries()) {
            ...
            if (pos == 0) {
                rCommand += "plot(density(" + data + "), " + "do.points=FALSE, " + "verticals=TRUE, " + "main=\""
                        + getDescription() + "\", " + "xlab=\"" + xAxisLabel + "\")\n"; // ,
                                                                                        // " + "ylab=\"f(t)\")\n";
            } else {
                rCommand += "lines(density(" + data + "),type=\"s\", lty=" + (pos + 1) + ")\n";
            }
            pos++;
        }
