...
public class REngineHelper {
     ...
     * with the standard {@link TransferType} which is <code>TransferType.MEMORY</code>.
     * 
     * @param measurements
     *            Measurements for a sensor.
     * @param sensorNumber
     *            number of the sensor vector in R.
     * @param dataSelection
     *            the data element to save.
     * @param rConnection
     *            Connection to the R engine.
     * @return R variable name which contains the data.
     */
    public static String storeMeasurementsInRVector(final SensorAndMeasurements measurements, final int sensorNumber,
    ...
    /**
     * Export the measurements of a sensor to R. There are two alternatives. The measurements can be
     * transferred via an array, which implies certain size restrictions. An alternative is to use a
     * temporary file. The behavior can only be switched in source code by the constant
     * <code>TRANSFER_TYPE</code>. Variable names in R are as follows:<br />
     ...
     */
    public static String storeMeasurementsInRVector(final SensorAndMeasurements measurements, final int sensorNumber,
            final TimeseriesData dataSelection, final TransferType transferType, final RConnection rConnection) {
        String sensorName = null;

        if (dataSelection == TimeseriesData.TIMESPAN) {
            sensorName = "sensor" + sensorNumber;
        } else if (dataSelection == TimeseriesData.EVENTTIME) {
        ...
        if (transferType == TransferType.MEMORY) {
            ...
            double[] measurementsArray = prepareExportToRByMemory(measurements, dataSelection);
        ...
        if (transferType == TransferType.FILE) {
            ...
            String rCommand = sensorName + " <- " + prepareExportToRByFile(measurements, dataSelection);
            Vector<REXP> result = rConnection.execute(rCommand);
            // Error handling
            if (!rConnection.getLastConsoleMessage().equalsIgnoreCase(
                    "Read " + measurements.getMeasurements().size() + " items\n")) {
                String rResults = "Executing command: '" + rCommand + "' with ";
                for (REXP currentResult : result) {
                    rResults += "String: " + currentResult.asString() + ", SymbolName: " + currentResult.asSymbolName()
                ...
                RVisualisationPlugin.log(IStatus.INFO,
                        "Storing Measurements in R via file is most likely wrong. Last message "
                                + "on the console was: " + rConnection.getLastConsoleMessage() + "R returned:\n"
                                + rResults);
            }
    ...
    /**
     * Prepares the export the measurements of a sensor to R. Therefore an array is filled with the
     * measurements.
     * 
     * @param measurements
     *            Measurements for a sensor.
     ...
     */
    private static double[] prepareExportToRByMemory(final SensorAndMeasurements measurements,
        ...
        if (measurements.getMeasurements().size() == Integer.MAX_VALUE) {
            RVisualisationPlugin.log(IStatus.ERROR, "Too many measurements. Results might be inaccurate.");
        }
        int position = 0;
        for (Measurement time : measurements.getMeasurements()) {
            TimeSpanMeasurement tsm = (TimeSpanMeasurement) time;
            measurementsArray[position++] = (dataSelection == TimeseriesData.EVENTTIME) ? tsm.getEventTime() : tsm
                    .getTimeSpan();
        }
    ...
    /**
     * Prepares to export the measurements of a time series sensor to R. Therefore a temporary file
     * is created and the R command line to import this data is returned.
     * 
     * @param measurements
     *            Measurements for a sensor.
     * @param dataSelection
     ...
     * @return R command to read measurements. Can be used to store data in a r vector.
     */
    private static String prepareExportToRByFile(final SensorAndMeasurements measurements,
            final TimeseriesData dataSelection) {
        File temporaryFile;
        try {
            temporaryFile = File.createTempFile("data", (dataSelection == TimeseriesData.EVENTTIME) ? "_et.txt"
                    : "_ts.txt");
            ...
            for (Measurement time : measurements.getMeasurements()) {
                TimeSpanMeasurement tsm = (TimeSpanMeasurement) time;
                result.append((dataSelection == TimeseriesData.EVENTTIME) ? tsm.getEventTime() : tsm.getTimeSpan());
                result.append(" ");
            }
            temporaryFileWriter.write(result.toString());
            temporaryFileWriter.close();
            return "scan(file=\"" + temporaryFile.getAbsolutePath().replace("\\", "\\\\") + "\")";
        } catch (IOException e) {
            RVisualisationPlugin.log(IStatus.ERROR, "Error accessing temporary file to transfer sensordata "
