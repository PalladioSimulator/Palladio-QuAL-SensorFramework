...
public class RConnectionImpl implements RConnection {
    ...
    private static final Logger LOGGER = Logger.getLogger(RConnectionImpl.class.getName());
    ...
    private static RConnectionImpl rConnection = null;
    ...
    protected void initalizeConnection() {
        ...
        try {
            if (!Rengine.versionCheck()) {
                ...
                RVisualisationPlugin.log(IStatus.ERROR, "Creating R engine ** Version mismatch - Java files (version "
                        + javaVersion + ") do not " + "match library version (version " + rniVersion + ").");
                new MessageDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Error loading R",
                        null, "Could not load R. The version of the java files (version " + javaVersion + ") and the "
                                + "library versions (version " + rniVersion + ") do not match.", MessageDialog.ERROR,
                        new String[] {
                            "OK"
                        }, 0).open();
            ...
            new MessageDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Error loading R", null,
                    "Could not load R 2.12. You need to install the correct "
                            ...
                            + "Check the error log for a detailed message.", MessageDialog.ERROR, new String[] {
                        "OK"
                    }, 0).open();
            return;
        }
    ...
    private void checkPackageAvailability(final String packageName) {
        ...
        if (!previousMessage.equals(result)) {
            RVisualisationPlugin.log(IStatus.ERROR, "Library \"" + packageName + "\" is not available. Please "
            ...
            // Get the ActiveWorkbenchWindow to send the error message to or handle it of there is
            // none.
            IWorkbenchWindow wbw = PlatformUI.getWorkbench().getActiveWorkbenchWindow();
            if (wbw != null) {
                new MessageDialog(wbw.getShell(), "Library \"" + packageName + "\" is not available in R", null,
                        "The library \"" + packageName + "\" is not available. " + "Please install the \""
                                + packageName + "\" package in " + "your R installation or"
                                + " the R reports will not work properly. Check the PDE "
                                + "error log for more information.", MessageDialog.ERROR, new String[] {
                            "OK"
                        }, 0).open();
            }
        }
    ...
    private void checkPathValidity() {
        ...
        if (!conflictingPaths.isEmpty()) {
            ...
            RVisualisationPlugin.log(IStatus.WARNING, "The environment variable java.library.path contains "
