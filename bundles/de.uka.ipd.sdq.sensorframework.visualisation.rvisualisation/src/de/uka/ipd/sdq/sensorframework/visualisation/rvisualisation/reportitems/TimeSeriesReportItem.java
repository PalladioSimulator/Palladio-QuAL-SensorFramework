...
public class TimeSeriesReportItem extends AbstractPlotReportItem {
    ...
    public TimeSeriesReportItem(final String title, final int height, final int width, final int fontSize,
    ...
    /**
     * Plots the first data set against the second. Expects to find the measured values in the first
     * set and the measurements number or event time in the second.
     */
    @Override
    protected String generatePlotCommand() {
        ...
        if (dataSeries.length > 1) {
            ...
            rCommand = "plot(" + xDataName + ", " + yDataName + ", main=\"" + getDescription() + "\", "
                    + "title=\"Histogram\", " + "xlab=\"" + xAxisLabel + "\", " + "ylab=\"Value\")\n";

        } else {
            RVisualisationPlugin.log(IStatus.ERROR,
                    "Internal Error: Less than two measurements in data for time series.");
        }
