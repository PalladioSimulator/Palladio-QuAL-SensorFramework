...
public abstract class AbstractPlotReportItem extends AbstractRReportItem {
    ...
    /**
     * Constructs a new report item containing a plotted density graphic. The graphic is stored in a
     * temporary file. This is accessible via the getFilename method.
     * 
     * @param title
     ...
     *            Title of the plotted graphic.
     * @param height
     *            height of the plotted graphic.
     * @param width
     *            width of the plotted graphics.
     * @param fontSize
     *            the default pointsize of plotted text, interpreted at 72 dpi, so one point is
     *            approximately one pixel.
     */
    public AbstractPlotReportItem(final String title, final int height, final int width, final int fontSize) {
        super(title);
     ...
     */
    public void addData(final String id, final String displayName, final String rVariable) {
     ...
     * 
     * @param pdfFile
     *            The file to which the graphic is saved.
     * @param rConnection
     *            The connection to the R engine.
     */
    public void generateVectorGraphic(final File pdfFile, final RConnection rConnection) {
        String rCommand = "";
        try {
            rCommand += "pdf(\"" + pdfFile.getAbsolutePath().replace("\\", "\\\\") + "\")\n";
            rCommand += generatePlotCommand();
            rCommand += "dev.off()\n";
        } catch (Exception e) {
            RVisualisationPlugin.log(IStatus.ERROR, "Could not create PDF file. " + e.getClass().getCanonicalName());
        }
     ...
     *            The connection to the R engine.
     */
    public void generateRasterGraphic(final File pngFile, final RConnection rConnection) {
        String rCommand = "";
        try {
            rCommand += "png(\"" + pngFile.getAbsolutePath().replace("\\", "\\\\") + "\",height=" + graphicsHeight
                    + ",width=" + graphicsWidth + ",pointsize=" + fontSize + ")\n";
            rCommand += generatePlotCommand();
            rCommand += "dev.off()\n";
        } catch (Exception e) {
            RVisualisationPlugin.log(IStatus.ERROR, "Could not create temporary graphic file. "
    ...
    public void generateData(final RConnection connection) {
        ...
        try {
            temporaryRasterGraphicFile = File.createTempFile("pcm_pic", ".png");
            temporaryRasterGraphicFile.deleteOnExit();
            generateRasterGraphic(temporaryRasterGraphicFile, connection);
        } catch (IOException e) {
            RVisualisationPlugin.log(IStatus.ERROR, "Could not create temporary raster graphic file. "
                    + e.getClass().getCanonicalName());
        }

        // generate vector image
        try {
            temporaryVectorGraphicFile = File.createTempFile("pcm_pic", ".pdf");
            temporaryVectorGraphicFile.deleteOnExit();
            generateVectorGraphic(temporaryVectorGraphicFile, connection);
        } catch (IOException e) {
            RVisualisationPlugin.log(IStatus.ERROR, "Could not create temporary vector graphic file. "
                    + e.getClass().getCanonicalName());
        }
    }
    ...
    protected String generateLinesLegend() {
        ...
        lines += ")";
        legend += ")";
        return "legend(\"" + legendPosition + "\", " + legend + ", lty=" + lines + ", bg='white')\n";
    }
    ...
    protected String generateColorsLegend() {
        ...
        fill += ")";
        legend += ")";
        return "legend(\"" + legendPosition + "\", " + legend + ", fill=" + fill + ", bg='white')\n";
    }
