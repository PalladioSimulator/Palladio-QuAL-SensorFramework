...
public class CdfReportItem extends AbstractPlotReportItem {
    ...
    /**
     * Constructs a new report item containing a plotted cdf graphic. The graphic is stored in a
     ...
     *            Label for the x axis.
     */
    public CdfReportItem(final String title, final int height, final int width, final int fontSize,
    ...
    protected String generatePlotCommand() {
        ...
        for (String id : getDataSeries()) {
            ...
            if (pos == 0) {
                plot += "plot(ecdf(" + sdata + "), xlim=r, " + "do.points=FALSE, " + "verticals=TRUE, " + "main=\""
                        + getDescription() + "\", " + "xlab=\"" + xAxisLabel + "\", " + "ylab=\"Cumulated Density\")\n";
            } else {
                plot += "lines(" + sdata + ", (1:length(" + sdata + "))/length(" + sdata + " ),type=\"s\", lty="
                        + (pos + 1) + ")\n";
            }
