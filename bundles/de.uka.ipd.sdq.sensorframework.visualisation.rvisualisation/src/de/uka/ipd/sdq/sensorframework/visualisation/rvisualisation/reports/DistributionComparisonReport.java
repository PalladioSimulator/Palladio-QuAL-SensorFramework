...
public class DistributionComparisonReport extends RReport {
     ...
     */
    @Override
    public ArrayList<IReportItem> prepareReportItems(final Collection<SensorAndMeasurements> c, final RConnection t) {
        ...
        PdfReportItem densityPlot = new PdfReportItem("Probability Density Function", "Time");
        HistogramReportItem histPlot = new HistogramReportItem("Histogram", "Time");
        ...
        for (int i = 0; i < sm.length; i++) {
            sm[i] = it.next();
            String sensorName = REngineHelper.storeMeasurementsInRVector(sm[i], i, TimeseriesData.TIMESPAN, t);

            densityPlot.addData(sm[i].getSensor().getSensorName(), "SensorID:" + sm[i].getSensor().getSensorID(),
                    sensorName);
            histPlot.addData(sm[i].getSensor().getSensorName(), "SensorID:" + sm[i].getSensor().getSensorID(),
                    sensorName);
            cdfPlot.addData(sm[i].getSensor().getSensorName(), "SensorID:" + sm[i].getSensor().getSensorID(),
                    sensorName);
            String rCommand = "length(" + sensorName + ")\n";
            items.add(new RCommandRReportItem(rCommand, "Number of observations of Sensor "
            ...
            items.add(new RCommandRReportItem(rCommand, "Standard-Deviation of Sensor "
                    + sm[i].getSensor().getSensorName()));
        }
        ...
        if (c.size() != 2) {
            ...
            items.add(new StaticTextReportItem("Comparing sensors " + sm[0].getSensor().getSensorName() + " and "
                    + sm[1].getSensor().getSensorName(), false));

            items.add(new StaticTextReportItem("KS test.", true));
            // t.execute("ks <- ks.test(sensor0,sensor1)\n ");

            items.add(new RCommandRReportItem("ks.test(sensor0,sensor1)$method", "The applied test for the sensors"));
            items.add(new RCommandRReportItem("ks.test(sensor0,sensor1)$statistic", "The value of the test statistics"));
            items.add(new RCommandRReportItem("ks.test(sensor0,sensor1)$p.value", "The p-value of the test"));

            items.add(new StaticTextReportItem("Chi^2 test.", true));
            // t.execute("chisq <- chisq.test(sensor0,sensor1)\n ");

            /*
             * The vectors must have the same length for the chi square test, so use the shorter
             * vector's length. Additionally, Chi square test cannot handle lots of data. Thus, only
             * compare the first 2500 values or less if the sensor contains less data.
             */
            int max = (sm[0].getMeasurements().size() < sm[1].getMeasurements().size() ? sm[0].getMeasurements().size()
                    : sm[1].getMeasurements().size());
            max = (max > 2500) ? 2500 : max;

            items.add(new RCommandRReportItem("chisq.test(sensor0[1:" + max + "],sensor1[1:" + max + "])$method",
                    "The applied test for the sensors"));
            items.add(new RCommandRReportItem("chisq.test(sensor0[1:" + max + "],sensor1[1:" + max + "])$statistic",
                    "The value of the test statistics"));
            items.add(new RCommandRReportItem("chisq.test(sensor0[1:" + max + "],sensor1[1:" + max + "])$p.value",
