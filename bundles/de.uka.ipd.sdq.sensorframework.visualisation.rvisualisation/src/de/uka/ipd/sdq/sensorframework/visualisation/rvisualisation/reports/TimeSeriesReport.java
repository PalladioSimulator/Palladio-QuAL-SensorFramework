...
public class TimeSeriesReport extends RReport {
    ...
    public ArrayList<IReportItem> prepareReportItems(final Collection<SensorAndMeasurements> c, final RConnection t) {
        ...
        HistogramReportItem hrt = new HistogramReportItem("Histogram", "Time");
        // hrt.setColumns(20);
        items.add(hrt);

        CdfReportItem cdf = new CdfReportItem("Cumulative Distribution Function", "Time");
        items.add(cdf);
        // cdf.setLegendPos(LegendPosition.right);

        PdfReportItem pdf = new PdfReportItem("Probability Density Function", "Time");
        items.add(pdf);
        // pdf.setLegendPos(LegendPosition.right);

        TimeSeriesReportItem timeSeries = new TimeSeriesReportItem("Time Series", "Event Time");
        ...
        for (int i = 0; i < c.size(); i++) {
            ...
            String sensorName = REngineHelper.storeMeasurementsInRVector(sm, i, TimeseriesData.TIMESPAN, t);

            String sensorNameEventTime = REngineHelper.storeMeasurementsInRVector(sm, i, TimeseriesData.EVENTTIME, t);

            items.add(new SummaryReportItem(sensorName, sm.getSensor().getSensorName()));
            ...
            items.add(new RCommandRReportItem(rCommand, "Mean of Sensor " + sm.getSensor().getSensorName()));
            ...
            rCommand = "length(" + sensorNameEventTime + ") / (max(" + sensorNameEventTime + ") - min("
                    + sensorNameEventTime + "))\n";
            items.add(new RCommandRReportItem(rCommand, "Throughput of Sensor " + sm.getSensor().getSensorName()));
