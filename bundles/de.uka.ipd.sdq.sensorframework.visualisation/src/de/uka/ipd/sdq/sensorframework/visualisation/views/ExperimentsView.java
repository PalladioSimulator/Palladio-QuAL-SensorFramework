...
public class ExperimentsView extends ViewPart {
    ...
    public void createPartControl(Composite parent) {
        ...
        Transfer[] transfers = new Transfer[] {
                EditorInputTransfer.getInstance(), LocalSelectionTransfer.getTransfer()
        };
    ...
    private void makeActions() {
        ...
        deleteDataSet = new Action() {
            ...
            public void run() {
                ...
                if (selectedExperiment != null && selectedFactory != null) {
                    selectedFactory.createExperimentDAO().removeExperiment(selectedExperiment, true);
                    viewer.refresh();
                }
            }
        };
        ...
        deleteDataSet.setImageDescriptor(VisualisationPlugin.getImageDescriptor("/icons/remove_obj.gif"));
     ...
     */
    private void exportSensorMeasurementToCSV(String fileName, Collection<Measurement> measurement, boolean isHeader,
            String separator) {
        ...
        try {
            ...
            if (measurement.iterator().next() instanceof StateMeasurement) {
                ...
                for (Iterator<Measurement> iterator = measurement.iterator(); iterator.hasNext();) {
                    ...
                    bufferedWriter.append(data.getEventTime() + separator + data.getSensorState().getStateLiteral()
                            + "\n");
                }
     ...
     *            will add a separator with more than one character).
     */
    private void exportExperimentRunToCSV(String pathDir, Collection<Sensor> sensors, ExperimentRun run,
        ...
        for (Iterator<Sensor> iterator = sensors.iterator(); iterator.hasNext();) {
            ...
            exportSensorMeasurementToCSV(pathFile, sensorAndMeasurements.getMeasurements(), isHeader, separator);
        }
    }
    ...
    private void exportSelectedDataToCSV() {
        ...
        if (object instanceof FileDAOFactory) {
            ...
            if (dialog.getValidPath()) {
                exportAllExperimentRunsToCSV(dialog.getPath(), treeContainer.getElements(), dialog.isHeader(),
                        dialog.getSeparator());
            }
            ...
            if (innerObject instanceof Sensor) {
                ...
                String fileName = sanitizeFileName(sensorAndMeasurements.getSensor().getSensorName().toString()
                        + ".csv");
                ...
                if (dialog.getValidPath()) {
                    exportSensorMeasurementToCSV(dialog.getPath(), sensorAndMeasurements.getMeasurements(),
                ...
                CSVSettingsDialog dialog = new CSVSettingsDialog("", dirExperiment, "", DialogType.DIRECTORY);

                if (dialog.getValidPath()) {
                    exportExperimentRunToCSV(dialog.getPath(), treeObject.getExperiment().getSensors(), run,
                            dialog.isHeader(), dialog.getSeparator());
                }
            }
        }
     ...
     *            will add a separator with more than one character).
     */
    private void exportAllExperimentRunsToCSV(String pathDir, Collection<TreeObject> allExperimentRuns,
            boolean isHeader, String separator) {
        ...
        for (Iterator<TreeObject> iterator = allExperimentRuns.iterator(); iterator.hasNext();) {
            ...
            if (innerObject instanceof ExperimentRun) {
                ...
                exportExperimentRunToCSV(pathAllExperimentRuns, treeObject.getExperiment().getSensors(), run, isHeader,
                        separator);
            }
        }
    }
