...
public class FilteredCollectionsManager {
    ...
    public Object getFilteredMeasurements(SensorAndMeasurements sam) {
        return new SensorAndMeasurements(sam.getSensor(), filteringTheMeasurements(sam.getMeasurements()));
    }
    ...
    private Collection<Measurement> filteringTheMeasurements(Collection<Measurement> measuremts) {
        ...
        if (factory.canFilter(measuremts, getProperty(factory.getProperties()))) {
            filteredCollection = factory.getFilteredCollection(measuremts, getProperty(factory.getProperties()));
        } else {
            // set the minimal value for filtering attribute
            filteredCollection = factory.getFilteredCollection(measuremts, factory.convertToType("0"));
        }
