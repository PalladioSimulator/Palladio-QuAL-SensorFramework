...
public class ParameterValueSerialiser implements ISerialiser {
    ...
    public ParameterValueSerialiser(ScalabilitySensor sSensor) {
        nbParameters = 1;
        /*
         * for (State s : stateSensor.getSensorStates()) { hashMap.put(s.getStateID(),s); }
         */
    }
    ...
    public Object[] deserialise(byte[] bytes) {
        ...
        for (int j = 0; j < values.length; j++) {
            ...
            // if (states[j] == null)
            // throw new
            // RuntimeException("State could not be deserialized as it returned null values for states.");
        }
